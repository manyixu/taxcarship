...
//		String sqlhphm_hpzl = "SELECT " + sql_con + " FROM SYJK_CCS_RKMX WHERE REFUSETYPE = 1 AND LOGGEDOUT = '0' AND TAXCONDITIONCODE <> 'R' AND HPHM = ? AND HPZL = ? ORDER BY SJCJRQ DESC" ;
		String sqlhphm_hpzl = "SELECT " + sql_con + " FROM SYJK_CCS_RKMX WHERE HPHM = ? AND HPZL = ? ORDER BY SJCJRQ DESC" ;
...
//		String sqlhphm_engineNoEndSix = "SELECT " + sql_con + " FROM SYJK_CCS_RKMX WHERE REFUSETYPE = 1 AND LOGGEDOUT = '0' AND TAXCONDITIONCODE <> 'R' AND HPHM = ? ORDER BY SJCJRQ DESC" ;
		String sqlhphm_engineNoEndSix = "SELECT " + sql_con + " FROM SYJK_CCS_RKMX WHERE HPHM = ? ORDER BY SJCJRQ DESC" ;
...
//		String sqlhphm_vinEndSix = "SELECT " + sql_con + " FROM SYJK_CCS_RKMX WHERE REFUSETYPE = 1 AND LOGGEDOUT = '0' AND TAXCONDITIONCODE <> 'R' AND HPHM = ? ORDER BY SJCJRQ DESC" ;
		String sqlhphm_vinEndSix = "SELECT " + sql_con + " FROM SYJK_CCS_RKMX WHERE HPHM = ? ORDER BY SJCJRQ DESC" ;
...
//		String sqlvinEndSix_engineNoEndSix = "SELECT " + sql_con + " FROM SYJK_CCS_RKMX WHERE REFUSETYPE = 1 AND LOGGEDOUT = '0' AND TAXCONDITIONCODE <> 'R' AND SUBSTR(VIN,-6,6) = ? AND SUBSTR(ENGINENO,-6,6) = ? ORDER BY SJCJRQ DESC" ;
		String sqlvinEndSix_engineNoEndSix = "SELECT " + sql_con + " FROM SYJK_CCS_RKMX WHERE SUBSTR(VIN,-6,6) = ? AND SUBSTR(ENGINENO,-6,6) = ? ORDER BY SJCJRQ DESC" ;
		// 5 根据“车架号”匹配车辆信息
...
//		String sqlVin = "SELECT " + sql_con + " FROM SYJK_CCS_RKMX WHERE REFUSETYPE = 1 AND LOGGEDOUT = '0' AND TAXCONDITIONCODE <> 'R' AND VIN = ? ORDER BY SJCJRQ DESC";
		String sqlVin = "SELECT " + sql_con + " FROM SYJK_CCS_RKMX WHERE VIN = ? ORDER BY SJCJRQ DESC";
		// 6 根据“17位车架号”匹配车辆信息
...
//		String sql_17vin = "SELECT " + sql_con + " FROM SYJK_CCS_RKMX WHERE REFUSETYPE = 1 AND LOGGEDOUT = '0' AND TAXCONDITIONCODE <> 'R' AND VIN = ? ORDER BY SJCJRQ DESC";
		String sql_17vin = "SELECT " + sql_con + " FROM SYJK_CCS_RKMX WHERE VIN = ? ORDER BY SJCJRQ DESC";
...
//					rkmxL = getRkmx_list(sqlVin,tax_date,vin);
					...
					// 根据匹配规则找对应车辆   2014-12-10 mili start
					rkmxL = filter_rkmx(rkmxL,vin,engineNo,hphm,flag_engineNo);
					if(rkmxL != null && rkmxL.size() > 0){
						// 根据找到的车辆信息 判断有木有缴纳税款
						rkmxL = filter_rkmx(rkmxL,tax_date);
						if(rkmxL != null && !rkmxL.isEmpty()){
							map.put("CARID", "5");
						}else{
							map.put("CARID", "");
						}
...
//						rkmxL = this.getRkmx_list(sqlVin,tax_date,vin_EndSix);
						...
						//2015-12-17	start	suobaowen	增加是否使用发动机号标志
						flag_engineNo = false ;
						//2015-12-17	end		suobaowen	增加是否使用发动机号标志
...
//						rkmxL = this.getRkmx_list(sql_17vin,tax_date);
		    		...
		    		// 根据匹配规则找对应车辆   2014-12-12 mili start
					rkmxL = filter_rkmx(rkmxL,vin,engineNo,hphm,flag_engineNo);
					if(rkmxL != null && rkmxL.size() > 0){
						// 根据找到的车辆信息 判断有木有缴纳税款
						rkmxL = filter_rkmx(rkmxL,tax_date);
...
//			rkmxL = this.getRkmx_list(sqlVin,tax_date,vin);
			...
			// 根据匹配规则找对应车辆   2014-12-12 mili start
			rkmxL = filter_rkmx(rkmxL,vin,null,hphm,flag_engineNo);
			if(rkmxL != null && rkmxL.size() > 0){
				// 根据找到的车辆信息 判断有木有缴纳税款
				rkmxL = filter_rkmx(rkmxL,tax_date);
				if(rkmxL != null && !rkmxL.isEmpty()){
...
//					&& "1".equals(code.getREMARK())) {
				map_code.put("CarMatchList", code.getCODEVALUE());
...
//				}
	...
	public List<SYJK_CCS_QGCLWTMDXX> getMatching_wtmd(Vehicle_Type vt){
		...
		if("1".equals(QNameListFlag)){ // 0=打开;1=关闭 MILI 增加问题名单开关 2014-11-13 10:33:29
			...
			// 条件字段
			String tableCondition = " VALIDATEFLAG = '1' ORDER BY SJCJRQ DESC";   //  mili 2015-1-29 16:21:45  去掉 AND
			// 若车架号为17位，则以17位车架号作为优先匹配，若匹配唯一       
			// mili 2015-1-29 15:56:11 更新后 问题名单17位VIN匹配规则：由原来的只用17位VIN码匹配，改成17位VIN码+发动机后六位一起匹配  ZLL  start
			if(vin.length() >= 17){
				if(!"".equals(engineNo) && engineNo.length() >= 6){
					engineNo = engineNo.substring(engineNo.length() - 6);
					querySql.append("SUBSTR(FDJH,-6,6) = '" + engineNo + "' AND");
				}else if(!"".equals(engineNo)){
					querySql.append(" FDJH = '" + engineNo + "' AND");
				}
				querySql.append(" CLSBDM = '" + vin + "' AND");
				querySql.append(tableCondition);
				wtmd_list = this.getWTMDList(querySql.toString());
				return wtmd_list ;
			// mili 2015-1-29 15:56:11 更新后 问题名单17位VIN匹配规则：由原来的只用17位VIN码匹配，改成17位VIN码+发动机后六位一起匹配  ZLL  end
				...
				if(!"".equals(hphm)){		// mili 去掉 为空的 值  2015-1-29 16:22:28
					querySql.append("' AND HPHM = '" + hphm);
				}
				if(!"".equals(hpzl)){		// mili 去掉 为空的 值  2015-1-29 16:22:28
					querySql.append("' AND HPZL = '" + hpzl);
				}
				querySql.append("' AND SUBSTR(FDJH,-6,6) = '" + engineNo + "' AND");
				querySql.append(tableCondition);
				wtmd_list = this.getWTMDList(querySql.toString());
				return wtmd_list ;
				// MILI 2014-11-6 新加一个匹配规则
				...
				if(!"".equals(vin)){	// mili 去掉 为空的 值  2015-1-29 16:22:28
					querySql.append(" CLSBDM = '" + vin + "' AND");
				}
				if(!"".equals(hphm)){	// mili 去掉 为空的 值  2015-1-29 16:22:28
					querySql.append(" HPHM = '" + hphm + "' AND");
				}
				if(!"".equals(hpzl)){	// mili 去掉 为空的 值  2015-1-29 16:22:28
					querySql.append(" HPZL = '" + hpzl + "' AND");
				}
				if(!"".equals(engineNo)){	// mili 去掉 为空的 值  2015-1-29 16:22:28
					querySql.append(" FDJH = '" + engineNo + "' AND");
				}
				querySql.append(tableCondition);
				wtmd_list = this.getWTMDList(querySql.toString());
...
//				qgclwtmdxx.setID(rs.getString("ID"));
				...
				qgclwtmdxx.setVALIDATEFLAG(rs.getString("VALIDATEFLAG"));
...
//				vv.setFuelType(rs.getString("FuelType"));
	...
	/**
	 * @author MILI
	 * @time 2014-12-10 11:00:59
	 * @描述：过滤入库明细
	 * */
	public List<SYJK_CCS_RKMX> filter_rkmx(List<SYJK_CCS_RKMX> rkmxL,String vin,String engineNo,String hphm,boolean flag_engineNo){
		List<SYJK_CCS_RKMX> list_rkmx = new ArrayList<SYJK_CCS_RKMX>();
		List<SYJK_CCS_RKMX> list_r = rkmxL;
		if (list_r != null && list_r.size() > 0) {
			for(int i = 0 ; i < list_r.size() ; i++){
				SYJK_CCS_RKMX rkmx = list_r.get(i);
				String _vin = rkmx.getVIN();
				String _engineNo = rkmx.getEngineNo();
//				String _hphm = rkmx.getHPHM();
				//2015-12-17	start	suobaowen	去掉号牌号码的判断条件，增加如果是匹配规则5只使用VIN匹配
				if(engineNo != null && !"".equals(engineNo) && flag_engineNo){
					if(vin.equals(_vin) && engineNo.equals(_engineNo)){
						list_rkmx.add(rkmx);
					}
				}else{
					if(vin.equals(_vin)){
				...
				//2015-12-17	end		suobaowen	去掉号牌号码的判断条件，增加如果是匹配规则5只使用VIN匹配
//				if(hphm == null || "".equals(hphm) || _hphm == null || "".equals(_hphm)){
//					if(vin.equals(_vin) && engineNo.equals(_engineNo)){
//						list_rkmx.add(rkmx);
//					}
//				}else{
//					if(vin.equals(_vin) && engineNo.equals(_engineNo) && hphm.equals(_hphm)){
//						list_rkmx.add(rkmx);
//					}
//				}
			}
	 ...
	 * @描述：过滤税款 
	 * */
	public List<SYJK_CCS_RKMX> filter_rkmx(List<SYJK_CCS_RKMX> rkmxL,String tax_date){
		List<SYJK_CCS_RKMX> list_rkmx = new ArrayList<SYJK_CCS_RKMX>();
		List<SYJK_CCS_RKMX> list_r = rkmxL;
//		Refusetype=1 and Loggedout='0' and TAXCONDITIONCODE<>'R' and paydate=to_char(sysdate,'yyyy')
		if (list_r != null && list_r.size() > 0) {
			for(int i = 0 ; i < list_r.size() ; i++){
				SYJK_CCS_RKMX rkmx = list_r.get(i);
				String Refusetype = rkmx.getRefusetype();
				String Loggedout = rkmx.getLOGGEDOUT();
				String TAXCONDITIONCODE = rkmx.getTAXCONDITIONCODE();
				String paydate = rkmx.getPAYDATE();
//				if("1".equals(Refusetype) && "0".equals(Loggedout) && !"R".equals(TAXCONDITIONCODE) && tax_date.equals(paydate)){
				if("1".equals(Refusetype) && "0".equals(Loggedout) && !"R".equals(TAXCONDITIONCODE)){
					list_rkmx.add(rkmx) ;
				}
			}
		}
